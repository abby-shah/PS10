...
```{r}
y_hattrain <- predict(LASSO_fit, newx=predictor_matrix_train, s=lambda_star_1SE) %>% 
  as.vector()

#TRAIN error
(y_hattrain - train$log_price_doc)^2 %>% 
  mean() %>% 
  sqrt()
...
--------------------- | -------------   | -------------
lm                    |     0.1200696   |  4.12045
LASSO crossvalidated  |     0.5055529   |  0.45162


The Kaggle Score for the Lm model is fairly high That's probably because we use very few observations to train our model. Naturally, the training score would be lower than the one computed from the out-of-sample test data on which the model is applied.
